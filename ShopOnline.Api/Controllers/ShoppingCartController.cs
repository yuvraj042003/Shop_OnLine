...
namespace ShopOnline.Api.Controllers
{
	...
	public class ShoppingCartController : ControllerBase
	{
		...
		public async Task<ActionResult<CartItemDto>> UpdateQty(int id, CartItemQtyUpdateDto cartItemQtyUpdateDto)
		{
			try
			{
				var carItem = await this.shoppingCartRepository.UpdateQty(id, cartItemQtyUpdateDto);
				if(carItem == null)
				{
					return NotFound();
				}
				var product = await productRepository.GetItem(carItem.ProductId);
				var cartItemDto = carItem.ConvertToDto(product);
				return Ok(cartItemDto);
			}
			catch (Exception ex)
			{
